...
object CodeProjectTestHelper {
  def getTestProject: \/[ModuleError,CodeProjectService] = {
...
class CodeProjectServiceTest extends FunSpec with Matchers {
  ...
  describe("a code project") {
    ...
    it("should read struct") {
      project.value.readStructure.value.files(0).path should be("src/main/scala/badlam/Playfield.scala")
    }
    describe("sample file") {
      val playfieldPath = project.value.readStructure.value.files(0).path
      val file = project.value.readFile(playfieldPath)
...
class  CodeProjectServiceAsyncTest extends AsyncFunSpec with Matchers {
  ...
  describe("a code project") {
    ...
    it("should not copy to non existing dest") {
      ...
      project.value.copyContentTo(nonExistingPath).unsafeToFuture map ( copyResult =>
    ...
    it ("should copy default project content") {
      ...
      project.value.copyContentTo(tempDir).unsafeToFuture map ( copiedPath =>
